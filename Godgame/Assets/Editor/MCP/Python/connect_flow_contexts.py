...
@mcp_for_unity_tool(
    description="Connect two VFX contexts via flow (spawn/init/update/output flow)"
)
async def connect_flow_contexts(
    ctx: Context,
    ...
    await ctx.info(f"Connecting flow contexts {source_context_id}:{source_slot_index} -> {target_context_id}:{target_slot_index} in {graph_path}")
    ...
    response = send_command_with_retry("connect_flow_contexts", params)
