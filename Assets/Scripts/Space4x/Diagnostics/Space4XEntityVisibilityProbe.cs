...
namespace Space4X.Diagnostics
{
    ...
    public sealed class Space4XEntityVisibilityProbe : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            _peak = default;
            _hasBaseline = false;
            UnityEngine.Debug.Log($"[Space4XEntityVisibilityProbe] boot active={_active} path='{_outputPath}'");
        }
        ...
        private void Update()
        {
            ...
            {
                _active = !_active;
                UnityEngine.Debug.Log($"[Space4XEntityVisibilityProbe] active={_active} path='{_outputPath}'");
                if (!_active)
                {
                    _hasBaseline = false;
                    _previous = default;
        ...
        private static bool TryAppendLine(string path, string line)
        {
            ...
            {
                UnityEngine.Debug.LogWarning($"[Space4XEntityVisibilityProbe] write failed: {ex.Message}");
                return false;
            }
        }
