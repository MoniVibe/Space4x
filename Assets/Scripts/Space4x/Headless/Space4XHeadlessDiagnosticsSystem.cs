        ...
        private void UpdateProgress(ref SystemState state, uint tick)
        {
            ...
            if (SystemAPI.TryGetSingleton<Space4XScenarioRuntime>(out var runtime))
            {
                ...
                if (_runStarted == 0 && tick >= runtime.StartTick)
                {
                    ...
                    Space4XHeadlessDiagnostics.UpdateProgress("run", "start", tick);UnityEngine.Debug.Log("[Anviloop] ARC_START_STUB");
                    Space4XHeadlessDiagnostics.UpdateProgress("run", "start", tick);UnityEngine.Debug.Log("[Anviloop] ARC_START_STUB");UnityEngine.Debug.Log("[Anviloop] ARC_START_STUB");
                }
