...
namespace Space4X.Authoring
{
    ...
    public sealed class Space4XCombatDemoAuthoring : MonoBehaviour
    {
        [Header("Player Fleet")]
        [SerializeField]
        ...
        private sealed class Baker : Unity.Entities.Baker<Space4XCombatDemoAuthoring>
        {
            private uint _entityCounter;

            public override void Bake(Space4XCombatDemoAuthoring authoring)
            {
                _entityCounter = 0;

                // Create config singleton
                var configEntity = GetEntity(TransformUsageFlags.None);
                AddComponent(configEntity, new CombatDemoConfig
                {
    ...
    /// <summary>
    /// Config singleton for combat demo.
    /// </summary>
    public struct CombatDemoConfig : IComponentData
    {
