...
namespace Space4X.Tests
{
    ...
    public class Space4XAreaEffectSystemTests
    {
        ...
        [Test]
        public void AreaEffect_StrengthAttenuation_PartialCoverReducesDamageWithoutFullBlock()
        {
            var emitter = _entityManager.CreateEntity(typeof(LocalTransform), typeof(Space4XAreaEffectEmitter));
            _entityManager.SetComponentData(emitter, new LocalTransform
            {
            ...
            _entityManager.SetComponentData(emitter, new Space4XAreaEffectEmitter
            {
                ...
                ImpactMask = Space4XAreaEffectImpactMask.HullDamage,
                OcclusionChannel = Space4XAreaOcclusionChannel.Blast,
                OcclusionMode = Space4XAreaOcclusionMode.StrengthAttenuation,
                DamageType = Space4XDamageType.Explosive,
                Radius = 20f,
                InnerRadius = 0f,
                Magnitude = 30f,
            ...
            var partialCoverTarget = _entityManager.CreateEntity(typeof(LocalTransform), typeof(HullIntegrity));
            _entityManager.SetComponentData(partialCoverTarget, new LocalTransform
            {
                Position = new float3(12f, 0f, 0f),
                Rotation = quaternion.identity,
                Scale = 1f
            });
            _entityManager.SetComponentData(partialCoverTarget, HullIntegrity.Create(100f));

            var openTarget = _entityManager.CreateEntity(typeof(LocalTransform), typeof(HullIntegrity));
            _entityManager.SetComponentData(openTarget, new LocalTransform
            {
                Position = new float3(0f, 12f, 0f),
                Rotation = quaternion.identity,
                Scale = 1f
            });
            _entityManager.SetComponentData(openTarget, HullIntegrity.Create(100f));

            var grazeOccluder = _entityManager.CreateEntity(typeof(LocalTransform), typeof(Space4XAreaOccluder));
            _entityManager.SetComponentData(grazeOccluder, new LocalTransform
            {
                Position = new float3(6f, 1.6f, 0f),
                Rotation = quaternion.identity,
                Scale = 1f
            });
            _entityManager.SetComponentData(grazeOccluder, new Space4XAreaOccluder
            {
                Radius = 2f,
                Strength01 = 1f,
                BlocksChannels = Space4XAreaOcclusionChannel.Blast
            ...
            var partialHull = _entityManager.GetComponentData<HullIntegrity>(partialCoverTarget);
            var openHull = _entityManager.GetComponentData<HullIntegrity>(openTarget);

            Assert.Less(partialHull.Current, 100f, "Partial cover should still allow some AoE damage through.");
            Assert.Greater(partialHull.Current, openHull.Current, "Partial cover should attenuate damage relative to an open lane target.");
        }

        [Test]
        public void AreaEffect_StrengthAttenuation_StackedOccludersAttenuateMore()
        {
            var emitter = _entityManager.CreateEntity(typeof(LocalTransform), typeof(Space4XAreaEffectEmitter));
            _entityManager.SetComponentData(emitter, new LocalTransform
            {
            ...
            var singleCoverTarget = _entityManager.CreateEntity(typeof(LocalTransform), typeof(HullIntegrity));
            _entityManager.SetComponentData(singleCoverTarget, new LocalTransform
            {
                Position = new float3(12f, 0f, 0f),
                Rotation = quaternion.identity,
                Scale = 1f
            });
            _entityManager.SetComponentData(singleCoverTarget, HullIntegrity.Create(100f));

            var stackedCoverTarget = _entityManager.CreateEntity(typeof(LocalTransform), typeof(HullIntegrity));
            _entityManager.SetComponentData(stackedCoverTarget, new LocalTransform
            {
                Position = new float3(-12f, 0f, 0f),
                Rotation = quaternion.identity,
                Scale = 1f
            });
            _entityManager.SetComponentData(stackedCoverTarget, HullIntegrity.Create(100f));

            var singleOccluder = _entityManager.CreateEntity(typeof(LocalTransform), typeof(Space4XAreaOccluder));
            _entityManager.SetComponentData(singleOccluder, new LocalTransform
            {
                Position = new float3(6f, 0f, 0f),
                Rotation = quaternion.identity,
                Scale = 1f
            });
            _entityManager.SetComponentData(singleOccluder, new Space4XAreaOccluder
            {
                Radius = 2f,
                Strength01 = 0.65f,
                BlocksChannels = Space4XAreaOcclusionChannel.Blast
            ...
            var stackedOccluderA = _entityManager.CreateEntity(typeof(LocalTransform), typeof(Space4XAreaOccluder));
            _entityManager.SetComponentData(stackedOccluderA, new LocalTransform
            {
                Position = new float3(-4f, 0f, 0f),
                Rotation = quaternion.identity,
                Scale = 1f
            });
            _entityManager.SetComponentData(stackedOccluderA, new Space4XAreaOccluder
            {
                Radius = 2f,
                Strength01 = 0.65f,
                BlocksChannels = Space4XAreaOcclusionChannel.Blast
            ...
            var stackedOccluderB = _entityManager.CreateEntity(typeof(LocalTransform), typeof(Space4XAreaOccluder));
            _entityManager.SetComponentData(stackedOccluderB, new LocalTransform
            {
                Position = new float3(-8f, 0f, 0f),
                Rotation = quaternion.identity,
                Scale = 1f
            });
            _entityManager.SetComponentData(stackedOccluderB, new Space4XAreaOccluder
            {
                Radius = 2f,
                Strength01 = 0.65f,
                BlocksChannels = Space4XAreaOcclusionChannel.Blast
            ...
            var singleHull = _entityManager.GetComponentData<HullIntegrity>(singleCoverTarget);
            var stackedHull = _entityManager.GetComponentData<HullIntegrity>(stackedCoverTarget);

            Assert.Less(singleHull.Current, 100f, "Single cover lane should still receive reduced AoE damage.");
            Assert.Greater(stackedHull.Current, singleHull.Current, "Stacked cover lane should receive less damage than single-cover lane.");
        }
