    ...
    public sealed class Space4XShotDirector : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            while (_nextBeatIndex < _beats.Length && worldSeconds >= _beats[_nextBeatIndex].time_s)
            {
                ...
                {
                    Debug.Log($"[Space4XShotDirector] scenario='{_scenarioId}' beat='{label}' time_s={beat.time_s:0.###}");
                }
                else
                {
                    Debug.Log($"[Space4XShotDirector] beat='{label}' time_s={beat.time_s:0.###}");
                }
        ...
        {
            var beatsPath = Environment.GetEnvironmentVariable(BeatsPathEnv);
            ...
            {
                Debug.LogWarning($"[Space4XShotDirector] beats file not found path='{resolvedPath}'");
                return;
            }
            ...
            try
            {
                ...
                {
                    Debug.LogWarning($"[Space4XShotDirector] beats file empty path='{resolvedPath}'");
                    return;
                }
                ...
                Debug.Log($"[Space4XShotDirector] beats_loaded=1 path='{resolvedPath}' count={_beats.Length}");
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[Space4XShotDirector] failed to parse beats path='{resolvedPath}' error='{ex.Message}'");
            }
        }
