...
namespace Space4X.Presentation.Overlay
{
    ...
    public sealed class Space4XEntityPicker : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (!_enabledInPlay || !Space4XOverlayInput.GetMouseButtonDown(0))
            {
                return;
            }

            if (Space4XOverlayInput.GetKey(KeyCode.LeftAlt) || Space4XOverlayInput.GetKey(KeyCode.RightAlt))
            {
                return;
            }
            ...
            if (!Space4XOverlayInput.TryGetMousePosition(out var mousePosition))
            {
                return;
            }
