...
**Goal**: Share a minimal extraction + production loop via PureDOTS, then adapt to Space4x modules/ship parts.

- **Extraction**: keep current mining loop; map output to production inputs.
- **Facilities**: carrier bay + station workshop as `ProductionFacility`.
