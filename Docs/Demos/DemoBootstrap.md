# Demo Bootstrap System

## Overview

The `DemoBootstrap` system provides a single entry point for demo scenes. It handles scenario selection, time controls, presentation toggles, and determinism overlays.
...
1. On scene load, `DemoBootstrap` scans `/Assets/Scenarios/<game>/*.json`
2. Displays list of available scenarios
3. User selects scenario (or uses default)
4. `DemoOptions` component created with selected path
...
### Render Sanity Ping (Space4X Demo)

- `DemoRenderSanitySystem` emits a single `PlayEffectRequest` with effect id `FX.Demo.Ping` and lifetime `2s` when the Space4X demo boots.
...
```csharp
// DemoBootstrap starts ScenarioRunner
...
```csharp
// DemoBootstrap controls time state
