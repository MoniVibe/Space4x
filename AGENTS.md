# Agents ΓÇô Role-First Guidelines
...
- **Space4X**: flagship game ΓÇö carriers as **villages in space** (hull, society, chain of command). Command ladder: `God ΓåÆ Admiral ΓåÆ Captain ΓåÆ Officer ΓåÆ Pilot`.
...
**Definition of Done:** Report run id, exit_reason, verdict (pass/fail), and top errors or suggested next step. Evidence order: `out/run_summary_min.json` ΓåÆ `out/run_summary.json` ΓåÆ `meta.json` ΓåÆ `out/watchdog.json` + logs.
...
- `PureDOTS/` ΓåÆ shared engine-level DOTS package (`Packages/com.moni.puredots`)
- `Godgame/` ΓåÆ game project using PureDOTS
- `Space4x/` ΓåÆ game project using PureDOTS
- `Foundation1/` and `Project1/` ΓåÆ read-only references unless user says otherwise
...
- Use canonical render pipeline: `<Game>RenderCatalogDefinition` ΓåÆ Baker ΓåÆ `RenderPresentationCatalog` + `RenderMeshArray` ΓåÆ `ResolveRenderVariantSystem` / `ApplyRenderVariantSystem` + presenter systems
- Respect unified RenderKey per game in Spawners, Rendering, Sanity systems

### Error Patterns (6.2)

- CS0118/CS0119 related to Camera ΓåÆ namespace hygiene; use aliases per NamespaceHygiene doc
- `SystemAPI.*` in static methods ΓåÆ move into `OnUpdate` or use `SystemState` APIs
- `GetSingleton<T>()` requires exactly one entity ΓåÆ ensure baked/created by canonical pipeline
- Deviations are design bugs, not tolerances

### Cross-OS Workflow (WSL + Windows) ΓÇö TRI multi-track
...
- No autoloop: implement between runs. Queue ΓåÆ diagnose ΓåÆ fix ΓåÆ requeue
- Evidence order: `out/run_summary_min.json` ΓåÆ `out/run_summary.json` ΓåÆ `meta.json` ΓåÆ `out/watchdog.json` + logs
